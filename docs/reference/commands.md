...
The `agent` check itself resolves against the full agent table, not a fixed list — a
[custom `[agents.<name>]`](configuration.md#custom-agents) resolves and is checked exactly like
a built-in. For an agent whose config declares no `integration` at all, `credentials` reports
`✓ none required` rather than `present` — nothing was checked because there was nothing to
check, which is a different fact from "checked and found."
...
| `--agent <AGENT>`, `-a` | Set the agent directly instead of asking — evaluated the same way with or without `--yes`. If it differs from the current value, it is written; if it matches, nothing is written. Must name an entry in the agent table — one of the four built-ins, or a `[agents.<name>]` section already defined in config; an unknown name is rejected immediately, before any file is touched, listing the configured agents. |
...
| Option | Description |
|---|---|
| `--agent <AGENT>` | Agent to launch in the session's agent pane. Overrides the `agent` value from config. Must name an entry in the agent table — one of the four built-ins (`claude`, `copilot`, `gemini`, `codex`), or a `[agents.<name>]` section from config. Unknown values are rejected with an error listing the configured agents. See [Concepts](../concepts.md#agent-integrations) and the [configuration reference](configuration.md#agentsname) for details, including how to define your own. |
...
2. **Window open, but the agent pane went idle** (the agent crashed, was killed, or exited) — relaunches the recorded agent (or restarts the container, for a container session) directly into the existing pane; the window and split are untouched. A host agent is typed in via `send-keys`; a container's run command is delivered with `tmux respawn-pane` instead, as a single unit rather than typed keystroke by keystroke, since it can be a multi-line script. Prints `Attached to session '<slug>'; agent had exited — relaunched '<agent>' (resuming).` (containerized: `; container had exited — restarted it.`)
