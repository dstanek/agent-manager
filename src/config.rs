...
# Applies only when container.mode resolves to "devcontainer". am builds the image once per
# config change and runs it itself, so nothing here is on the hot path.
[devcontainer]
